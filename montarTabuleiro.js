$(function(){
	...
	let elementLeave = null
	$('.casa').on('dragleave', function(evt) {
		elementLeave = evt.currentTarget
	});
	
	$('.casa').on('dragend', function(evt) {
		console.log(evt)
		$(elementLeave).click();
	});
	
	$('.casa').on('dragstart', function(evt) {
...
function click(e){
	...
	if(peca_selecionada != null){
		...
		nI = parseInt($(e).attr("id").substr(5, 1));
		nJ = parseInt($(e).attr("id").substr(7, 1));
